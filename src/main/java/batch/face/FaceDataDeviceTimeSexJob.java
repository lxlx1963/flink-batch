...
import com.xinchao.flink.model.dto.FaceDataDeviceTimeSexDTO;
 ...
 */
public class FaceDataDeviceTimeSexJob {
	/**
	 * 日志
	 */
	private static Logger logger = LoggerFactory.getLogger(FaceDataDeviceTimeSexJob.class);
	...
	public static void main(String[] args) {
		try {
			...
			Table joinTable = peopleTimeTable.leftOuterJoin(peopleNumberTable).where("deviceCode = tempDeviceCode")
					...
					.select("deviceCode, dateTime, gender, residenceType, peopleNumber, peopleTime");

			// 昨天的日期
			String yearMonthDay = DateUtils.getPastDay(1, DateConstant.DATE_YEAR_MONTH_DAY);
			// 插入SQL
			String insertSql = "INSERT INTO frp_face_data_device_time_age(date, device_code, residence_type, time, sex, people_num, people_time, add_time) VALUES (?,?,?,?,?,?,?,?)";

			// 转为为Row
			MapOperator<FaceDataDeviceTimeSexDTO, Row> resultMap = batchTableEnvironment.toDataSet(joinTable, FaceDataDeviceTimeSexDTO.class)
					.map(dto -> {
						Row row = new Row(8);
						...
						row.setField(4, GenderUtils.genderToSex(gender));
						row.setField(5, dto.getPeopleNumber());
						row.setField(6, dto.getPeopleTime());
						long currentTime = DateUtils.getCurrentTimestamp().getTime();
						row.setField(7, currentTime);
						return row;
					});
			...
			// 执行任务
			env.execute("设备小时性别维度统计人数、人次");
		} catch (Exception e) {
			logger.error("设备小时性别维度统计人数、人次", e);
		}
	}
	...
	private static Table getPeopleTimeTable(BatchTableEnvironment batchTableEnvironment) {
		// 人次SQL
		String peopleTimeSql = "SELECT deviceCode, dataTime, gender, COUNT(id) as peopleTime FROM FaceData GROUP BY deviceCode, dataTime, gender";
		// 人次Table
	...
	private static Table getPeopleNumberTable(BatchTableEnvironment batchTableEnvironment) {
		...
		String peopleNumberSql = "SELECT " +
				...
				"FROM " +
				"( SELECT deviceCode, dataTime, gender, visitorId FROM FaceData GROUP BY deviceCode, dataTime, gender, visitorId ) temp " +
				"GROUP BY " +
				"temp.deviceCode," +
				"temp.dataTime," +
				"temp.gender";

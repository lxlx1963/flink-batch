...
import com.xinchao.flink.model.dto.FaceDataDeviceTimeAgeDTO;
 ...
 */
public class FaceDataDeviceTimeAgeJob {
	/**
	 * 日志
	 */
	private static Logger logger = LoggerFactory.getLogger(FaceDataDeviceTimeAgeJob.class);
	...
	public static void main(String[] args) {
		try {
			...
			Table joinTable = peopleTimeTable.leftOuterJoin(peopleNumberTable).where("deviceCode = tempDeviceCode")
					...
					.select("deviceCode, dateTime, ageRange, residenceType, peopleNumber, peopleTime");

			// 昨天的日期
			String yearMonthDay = DateUtils.getPastDay(1, DateConstant.DATE_YEAR_MONTH_DAY);
			// 插入SQL
			String insertSql = "INSERT INTO frp_face_data_device_time_age(date, device_code, residence_type, time, age, people_num, people_time, add_time) VALUES (?,?,?,?,?,?,?,?)";

			// 转为为Row
			MapOperator<FaceDataDeviceTimeAgeDTO, Row> resultMap = batchTableEnvironment.toDataSet(joinTable, FaceDataDeviceTimeAgeDTO.class)
					.map(dto -> {
						Row row = new Row(8);
						...
						row.setField(3, hour);
						row.setField(4, dto.getAgeRange());
			...
			// 执行任务
			env.execute("设备小时年龄维度统计人数、人次");
		} catch (Exception e) {
			logger.error("设备小时年龄维度统计人数、人次", e);
		}
	}
	...
	private static Table getPeopleTimeTable(BatchTableEnvironment batchTableEnvironment) {
		// 人次SQL
		String peopleTimeSql = "SELECT deviceCode, dataTime, ageRange, COUNT(id) as peopleTime FROM FaceData GROUP BY deviceCode, dataTime, ageRange";
		// 人次Table
	...
	private static Table getPeopleNumberTable(BatchTableEnvironment batchTableEnvironment) {
		...
		String peopleNumberSql = "SELECT " +
				...
				"FROM " +
				"( SELECT deviceCode, dataTime, ageRange, visitorId FROM FaceData GROUP BY deviceCode, dataTime, ageRange, visitorId ) temp " +
				"GROUP BY " +
				"temp.deviceCode," +
				"temp.dataTime," +
				"temp.ageRange";

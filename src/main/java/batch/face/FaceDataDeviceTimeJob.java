 ...
 */
public class FaceDataDeviceTimeJob {
	/**
	 * 日志
	 */
	private static Logger logger = LoggerFactory.getLogger(FaceDataDeviceTimeJob.class);
	...
	public static void main(String[] args) {
		try {
			...
			Table joinTable = peopleTimeTable.leftOuterJoin(peopleNumberTable).where("deviceCode = tempDeviceCode")
					...
					.select("deviceCode, dateTime, residenceType, peopleNumber, peopleTime");

			// 昨天的日期
			String yearMonthDay = DateUtils.getPastDay(1, DateConstant.DATE_YEAR_MONTH_DAY);
			// 插入SQL
			String insertSql = "INSERT INTO frp_face_data_device_time(date, device_code, residence_type, time, people_num, people_time, add_time) VALUES (?,?,?,?,?,?,?)";
			...
			MapOperator<FaceDataDeviceTimeAgeDTO, Row> resultMap = batchTableEnvironment.toDataSet(joinTable, FaceDataDeviceTimeAgeDTO.class)
					.map(dto -> {
						...
						row.setField(3, hour);
						row.setField(4, dto.getPeopleNumber());
						row.setField(5, dto.getPeopleTime());
			...
			// 执行任务
			env.execute("设备小时维度统计人数、人次");
		} catch (Exception e) {
			logger.error("设备小时维度统计人数、人次", e);
		}
	}
	...
	private static Table getPeopleTimeTable(BatchTableEnvironment batchTableEnvironment) {
		// 人次SQL
		String peopleTimeSql = "SELECT deviceCode, dataTime, COUNT(id) as peopleTime FROM FaceData GROUP BY deviceCode, dataTime";
		// 人次Table
	...
	private static Table getPeopleNumberTable(BatchTableEnvironment batchTableEnvironment) {
		...
		String peopleNumberSql = "SELECT " +
				...
				"FROM " +
				"( SELECT deviceCode, dataTime, visitorId FROM FaceData GROUP BY deviceCode, dataTime, visitorId ) temp " +
				"GROUP BY " +
				"temp.deviceCode," +
				"temp.dataTime";
		// 人数Table
